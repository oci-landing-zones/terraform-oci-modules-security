...
resource "oci_bastion_bastion" "these" {
    ...
    precondition {
      ...
      error_message = "VALIDATION FAILURE in bastion \"${each.key}\": the session time to live must be between 1800s(30min) and 10800s(3h). Please change the max_session_ttl_in_seconds value."
    }
    ## Check 2: Check if the cidr_block_allow_list is set
    precondition {
