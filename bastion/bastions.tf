...
resource "oci_bastion_bastion" "these" {
  for_each = var.bastions_configuration != null ? var.bastions_configuration["bastions"] : {}
    ...
    ## Check 1: Check if the value of the session time to live.
      ...
      precondition {
        condition     = coalesce(var.bastions_configuration.enable_cidr_check,true) ? (each.value.cidr_block_allow_list != null ? (contains(each.value.cidr_block_allow_list,"0.0.0.0/0") ? false : true ) : var.bastions_configuration.default_cidr_block_allow_list != null (contains(var.bastions_configuration.default_cidr_block_allow_list,"0.0.0.0/0") ? false : true)) : true
    ...
    bastion_type     = upper(coalesce(each.value.bastion_type,"standard"))
    ...
    dns_proxy_status = coalesce(each.value.enable_dns_proxy,true) ? "ENABLED" : "DISABLED"
